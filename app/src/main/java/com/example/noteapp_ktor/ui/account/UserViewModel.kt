package com.example.noteapp_ktor.ui.account
...
class UserViewModel @Inject constructor(
    ...
    fun createUser(
        ...
        if(!isPasswordValid(password)){
            _registerState.emit(Result.Error("Password should be between ${Constants.MINIMUM_PASSWORD_LENGTH} and ${Constants.MAXIMUM_PASSWORD_LENGTH}"))
            return@launch
        }
    ...
    fun loginUser(
        ...
        if(!isPasswordValid(password)){
            _loginState.emit(Result.Error("Password should be between ${Constants.MINIMUM_PASSWORD_LENGTH} and ${Constants.MAXIMUM_PASSWORD_LENGTH}"))
            return@launch
        }
    ...
    private fun isPasswordValid(password: String):Boolean {
        ...
        return (password.length in Constants.MINIMUM_PASSWORD_LENGTH..Constants.MAXIMUM_PASSWORD_LENGTH)
